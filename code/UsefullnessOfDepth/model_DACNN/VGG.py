...
from .ops.depthconv.module import DepthConv
from .ops.depthavgpooling.module import DepthAvgPooling
...
class DepthConvModule(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, dilation=1,bn=False, alpha=8.3):
        super(DepthConvModule, self).__init__()

        conv2d = DepthConv(inplanes,planes,kernel_size=kernel_size,alpha=alpha,stride=stride,padding=padding,dilation=dilation)
        layers = []
        if bn:
            layers += [nn.BatchNorm2d(planes), nn.ReLU(inplace=True)]
        else:
...
class VGG(nn.Module):
    ...
    def __init__(self, num_classes=20, init_weights=True, depthconv=False,bn=False):
        ...
        print("VGG with depthconv: ",depthconv)
