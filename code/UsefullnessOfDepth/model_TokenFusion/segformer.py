...
class WeTr(nn.Module):
    def __init__(self, cfg, embedding_dim=256, pretrained=True):
        super().__init__()
        ...
        self.encoder = getattr(mix_transformer, cfg.backbone)()
        print(f"Encoder: {cfg.backbone}")
        ...
        if pretrained:
            ...
            self.encoder.load_state_dict(state_dict, strict=False) # From strict=True to strict=False
    ...
    def get_param_groups(self, lr, weight_decay):
        ...
        params = [
            ...
            {"params": param_groups[1], "lr": lr, "weight_decay": 0.0},
            {"params": param_groups[2], "lr": lr * 10, "weight_decay": weight_decay},
        ]
        return params

    def forward(self, x_rgb, x_e):
        # Convert x_e from 1 channel to 3 channels
        if len(x_rgb.shape)==3:
        ...
        x = [F.interpolate(x_, size=x_rgb.size()[-2:], mode='bilinear', align_corners=False) for x_ in x]
