...
from simple_dataloader import get_train_loader, get_val_loader
        ...
        if (epoch%config.checkpoint_step==0 and epoch>int(config.checkpoint_start_epoch)):
            ...
            with torch.no_grad():
                ...
                for idx in pbar:
                    ...
                    images_eval = images_eval.cuda(non_blocking=True)
                    labels_eval = labels_eval.cuda(non_blocking=True)
                    modal_xs_eval = modal_xs_eval.cuda(non_blocking=True)
